...
func (l *GetChatLogLogic) GetChatLog(in *im.GetChatLogReq) (*im.GetChatLogResp, error) {
	...
	if in.MsgId != "" {
		...
		if err != nil {
			return nil, errors.Wrapf(xerr.NewDBErr(), "find chatlog by msg_id err %v, req %v", err, in.MsgId)
		}
	...
	if err != nil {
		return nil, errors.Wrapf(xerr.NewDBErr(), "find chatlog set by send_time err %v, req %v", err, in.MsgId)
	}
